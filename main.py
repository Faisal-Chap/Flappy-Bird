...
lives = 3
heart_image = pygame.image.load("/media/faisal-chap/Python/p-projects/Flappy Bird/heart.png")
heart_image = pygame.transform.scale(heart_image, (30, 30))

bird_image = pygame.image.load("assests/f-bird.png").convert_alpha()
bird_rect = bird_image.get_rect(center=(200, 300))

instruction_img = pygame.image.load("assests/intro.png").convert_alpha()
instruction_img = pygame.transform.scale(instruction_img, (1020, 800))  # or adjust to fit your screen
...
while True:
    if game_state == "start":
        ...
        continue  # skip rest of game loop until SPACE is pressed
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        ...
        if event.type == SPAWN_PIPE_EVENT and not paused and not game_over:
            gap_y = random.randint(150, SCREEN_HEIGHT - ground_height - 150)
            pipes.append({'x': SCREEN_WIDTH, 'gap_y': gap_y})

    if score < 1500 or (2500 <= score < 4000):
        current_bg = day_bg
    ...
    current_pipe_color = (90, 0, 120) if score < 1500 or (2500 <= score < 4000) else (25, 25, 112)
